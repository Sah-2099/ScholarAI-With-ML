...
import quizService from '../../services/quizService'; // ✅ Keep this
...
const QuizManager = ({ documentId }) => {
  ...
  const fetchQuizzes = async () => {
    ...
    try {
      ...
      const data = Array.isArray(response) ? response : (response.data || response.quizzes || []);
      setQuizzes(data);
    } catch (error) {
  ...
  const handleGenerateQuiz = async (e) => {
    ...
    setGenerating(true);
    try {
      // Generate via backend - this will save to MongoDB automatically
      await aiService.generateQuiz(documentId, { numQuestions });
      
      toast.success('Quiz generated successfully!');
      setIsGenerateModalOpen(false);
      setNumQuestions(5);
      fetchQuizzes(); // Refresh from backend
    } catch (error) {
      console.error('Quiz generation failed:', error);
      const errorMessage = error.message || 'Failed to generate quiz. Try again.';
