...
const Flashcard = ({ flashcard, onToggleStar }) => {
  // 🔒 Guard against undefined or null flashcard
  if (!flashcard) {
    return (
      <div className="relative w-full h-72 flex items-center justify-center bg-slate-100 rounded-2xl border-2 border-dashed border-slate-300">
        <p className="text-slate-500 text-sm">Loading flashcard...</p>
      </div>
    );
  }
  ...
  // Use optional chaining and fallbacks for all flashcard properties
  const { _id, difficulty = 'medium', isStarred = false, question = '', answer = '' } = flashcard;
