...
const DashboardPage = () => {
  ...
  const recentActivityItems = [
    ...
    ...(dashboardData.recentActivity.quizzes || []).map(quiz => ({
      ...
      timestamp: quiz.completedAt, // ✅ Correct field
    ...
    .filter(item => item.timestamp) // Only include items with valid timestamps
    .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
    .slice(0, 10); // Optional: limit total items
              ...
              {recentActivityItems.map((activity) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      <div
                        className={`w-2 h-2 rounded-full ${
                          activity.type === 'document' ? 'bg-blue-400' : 'bg-emerald-400'
                        }`}
                      />
                      <p className="text-sm font-medium text-slate-900 truncate">
                        {activity.type === 'document'
                          ? `Accessed Document: ${activity.description}`
                          : `Attempted Quiz: ${activity.description} — ${activity.score ?? '?'}${
                  ...
                  {activity.link && (
                    <a
                      href={activity.link}
                      className="ml-4 px-4 py-2 text-xs font-medium text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg transition-all duration-200 whitespace-nowrap"
                    >
                      View
                    </a>
                  )}
