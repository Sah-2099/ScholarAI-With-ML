...
const DocumentListPage = () => {
  ...
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false); // ✅ Fixed name
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      setUploadFile(null);
      setUploadTitle("");
      fetchDocuments(); // No need to setLoading(true) here
    } catch (error) {
  ...
  const handleDeleteRequest = (doc) => {
    setSelectedDoc(doc);
    setIsDeleteModalOpen(true); // ✅ Consistent name
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setIsDeleteModalOpen(false); // ✅ Consistent name
      setSelectedDoc(null);
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative max-w-7xl mx-auto p-6"> {/* Added padding for better spacing */}
        {/* Header */}
      ...
      {isUploadModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-slate-900/20 p-8"> {/* ✅ bg-white fixed */}
            <button
                ...
                >
                  {uploading ? (
                    <span className="flex items-center justify-center gap-2">
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> {/* ✅ animate-spin */}
                      Uploading...
                    </span>
                  ) : (
                    "Upload"
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8">
            <button
              onClick={() => setIsDeleteModalOpen(false)} // ✅ Consistent
              className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all duration-200"
            >
              <X className="w-5 h-5" strokeWidth={2} />
            </button>
