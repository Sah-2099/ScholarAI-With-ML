...
export const chunkText = (text, chunkSize = 500, overlap = 50) =>{
    ...
    // Clean text while prserving paragraph structure
    ...
    // Try to slipt by paragraph (single or double newlines)
    const paragraph = cleanText.split(/\n+/).filter(p=> p.trim().length > 0);
    ...
    // if adding this paragraph execeeds chunk size, save current chunk
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
    ...
    const scoredChunks = chunks.map((chunk,index) =>{
        ...
        const uniqueWordsFound = queryWords.filter(wprd =>
