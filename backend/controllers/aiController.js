import Documment from '../models/Document.js';
...
export const generateFlashcards = async (req, res, next) => {
    try {
        ...
        const document = await Documment.findOne({
            _id: documentId,
            userId: req.user._id,
            status: 'ready'
        });
...
export const generateSummary = async (req, res, next) => {
 try {
    ...
    if(!document) {
        return res.status(404).json({
            success:false,
            error: 'Documment not found or not ready',
            statusCode: 404
        });
    }
...
export const chat = async (req, res, next) => {
 try {
    ...
    if(!document || !question) {
        return res.status(400).json({
            success: false,
            error: 'Please provide documentId adn question',
            statusCode: 400
        });
    }
...
export const explainConcept = async (req, res, next) => {
 try {
    ...
    const context = relevantChunks.map (c => c.content).json('\n\n');
